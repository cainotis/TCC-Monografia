...
\documentclass[
  ...
  english, brazilian, 12pt, aspectratio=149,
]{beamer}
...
\graphicspath{{figuras/apresentacao},{logos/}}
...
\AtBeginSection[]{
  \begin{frame}<beamer>[t]{Visão Geral}
    \intermezzo
  \end{frame}
}
...
\title[Aprendizado por reforço no Duckietown]{Estudo de caso em aprendizado por reforço no universo Duckietown}

\author[Cainã S. Galante.]{Cainã Setti Galante}

\institute{\textbf{MAC0499 - Trabalho de Formatura Supervisionado}\\Departamento de Ciência da Computação\\IME USP}
\institute{\textbf{Orientador:} Denis Deratani Mauá\\Departamento de Ciência da Computação\\IME USP}
